...
soundfile = function () {
   ...
   *  function draw() {
   *    background(0);
   *    packageBody.x = constrain(mouseX, 0, width);
   *    ellipse(packageBody.x, height/2, 20, 20)
   *  }
   *
   *  function mousePressed(){
   *    // map the packageBody's x location to a panning degree
   *    // between -1.0 (left) and 1.0 (right)
   *    let panning = map(packageBody.x, 0., width,-1.0, 1.0);
...
reverb = function () {
   ...
   *  generate an impulse response is to pop a packageBodyoon in a reverberant space
